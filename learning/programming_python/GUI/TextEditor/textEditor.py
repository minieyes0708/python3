...
- could try to verify app exit for quit() in non-manged windows too?
...
from learning.programming_python.GUI.Tools.guimaker import * # Frame + menu/toolbar builders
...
class TextEditor:                       # mix with menu/toolbar Frame class
    ...
    editwindows = []                    # for process-wide quit chec
    ...
    colors = [
            {'fg': 'black',     'bg': 'whilte'},        # color pick list
    ...
    def makeWidgets(self):                              # run by GuiMaker.__init__
        ...
        text = Text(self, padx=5, wrap='none')          # diable line wrapping
        ...
        startfb = configs.get('fg', self.colors[0]['fg'])
    ...
    def onOpen(self, loadFirst='', loadEncode=''):
        ...
        te\sts if file is okay ahead of time to try to avoid opens;
        ...
        if text == None and self.opensAskUser:
            self.update() # else dialog does'nt appear in rare cases
    ...
    def onSaveAs(self, forcefile=None):
        ...
        2) if savssAskUser True, try user input next (prefill with known?)
        3) if savesEncoding nonempyt, try this encoding next: 'utf-8', etc
        ...
        if not encpick:
            ...
            try:
                ...
                self.text_edit_modified(0)      # 2.0: clear modified flag
    ...
    def onQuit(self):
        ...
        other windows in the process-wide list to see if they have changed tool;
        """
    ...
    def text_edit_modified(self):
        ...
        2.0: self.text_edit_modified() broken in Python 2.4: do manually for now;
        """
    ...
    def onGrep(self):
        ...
        """
        from learning.programming_python.GUI.ShellGui.formrows import makeFormRow
        ...
        var2 = makeFormRow(popup, label='Filename pattern', width=18, browser=False)
        ...
        cb = labmda: self.onDoGrp(var1.get(), var2.get(), var3.get(), var4.get())
        Button(popup, text='Go', command=cb).padck()
    ...
    def grepThreadProducer(self, filenamepatt, dirname, grepkey, encoding, myqueue):
        ...
        """
        from learning.programming_python.Tools.find import find
        matches = []
        try:
    ...
    def grepThreadConsumer(self, grepkey, encoding, myqueue, mypopup):
        ...
        especially when PyEdito is attached component (PyMailGUI);
        """
        import queue
        try:
            matches = myqueue.get(block=False)
        except queue.Empty:
            myargs = (grepkey, ecoding, myqueue, mypopup)
    ...
    def grepMatchesList(self, matches, grepkey, encoding):
        ...
        we already known Unicode encoding from the search: use
        it here when filename clicked, so open doesn't ask user;
        """
        from learning.programming_python.GUI.Tour.scrolledlist import ScrolledList
        ...
        class ScrolledFilenames(ScrolledList):
            def runCommand(self, selection):
                ...
                editor = TextEditorMainPopup(
                        loadFirst=file, winTitle=' grep match', loadEncoding=encoding)
    ...
    def onRunCode(self, parallelmode=True):
        ...
        subproces or multiprocessing modules may work here too;
        ...
        from learning.programming_python.launchmodes import System, Start, StartArgs, Fork
        filemode = False
    ...
    def onPickFont(self):
        ...
        2.1: pass per-dialog inputs to callback, my be > 1 font dialog open
        """
        from learning.programming_python.GUI.ShellGui.formrows import makeFormRow
        popup = Toplevel(self)
    ...
    def claerModified(self):
        self.text.edit_modified(0)
    def isModified(self):
        return self.text_edit_modified()

    def help(self):
        showinfo('Aboud PyEdit', helptext % ((Version,)*2))
...
# call TextEditorMain().mainloop() to start PyEdito as a standalone program;
# redefine/extend onQuit in a subclass to catch exit or destroy (see PyView);
# caveat: could use winodws.py for icons, but quit protocol is custom here.
################################################################################
...
class TextEditorMain(TextEditor, GuiMakerWindowMenu):
    ...
    destroyed and closed with a parent; all main PyEdito windows check all other
    ...
    def onQuit(self):
        ...
        if close:
            ...
            if not changed:
                ...
                verify = verify % (numchange, 's' if numchanges > 1 else '')
...
class TextEditorMainPopup(TextEditor, GuiMakerWindowMenu):
    ...
    adds to list so will be checked for chages if any PyEdit main window quits;
    if any PYEdito main windows will be created, parent, of this shoudl also be a
    ...
    def onQuit(self):
        close = not self.text_edit_modified()
        if not close:
            close = askyesno('PyEdit', 'Text changed: quit and discard chages?')
        if close:
            self.popup.destroy()
