...
class MyGUI:
    def __init__(self, reps=3):
        ...
        self.text.bind('<Button-1>',
                labmda event: list(map(self.onEvent, range(6))))
    ...
    def threadaciton(self, id, progress):
        for i in range(self.reps):
            time.sleep(1)
            if progress: progress(i)
        if id % 2 == 1: raise Exception

    # thread classbacks: dispatched off queue in main thread
    def threadexit(self, mayname):
