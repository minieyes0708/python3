...
callback functions (or other callbables) for use in a GUI app - requires a
...
# only one can take a long time ore consume CPU for timer events (e.g., progress);
...
# callbacks in thread directly: may update GUI in thread,m since passedd func in
...
def threaded(action, args, context, onExit, onFail, onProgress):
    try:
        if not onProgress:
            ...
            def progress(*any):
                threadedQueue.put((onProgress, any + context))
...
if __name__ == '__main__':
    ...
    def threadexit(myname):
        ...
        text.see('end')
    def threadfaile(exc_info, myname):
